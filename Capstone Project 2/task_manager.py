...
def login():
    ...
    while not logged_in:        
        ...
        # -- User restricted fron access if username is non-existenet 
        ...
        if curr_user_input not in username_password.keys():
            ...
            logged_in = True
            # -- Current user (global variable) now becomes the user logged on
            curr_user = curr_user_input
            # -- Once user is successfully logged on, menu function is called
            menu()

# -- Function to verify if logged in user is admin    
def admin_privillage():
    has_privillage = False
    if curr_user == 'admin':
        has_privillage = True
        
    return has_privillage

# -- Function that retrives all usernames and passwords from user.txt file
...
def confirm_password(new_password,confirm_password):
    confirmed = False
...
def add_task():
    ...
    # -- Dictionary with usernames and passwords 
    username_password = read_users()
    # -- Number of unsuccessful attemps to create a task
    ...
    while True:
        ...
        with open("tasks.txt", "a") as task_file:
            task_list_to_write = []
            for t in task_list:
                str_attrs = [str(t['id']),t['username'],t['title'],t['description'],t['due_date'].strftime(DATETIME_STRING_FORMAT),
...
def user_verifier(uname,pswrd):
    ...
    if username_password[uname] == pswrd:
        user_verified = True
...
def view_my_task():
    ...
    while True:
        # -- Prompts user to enter their username and passpord for verification
        ...
        if user_verifier(username_entered,password_entered):
            ...
            print("Username doesn't exist or one or both username,password is/are incorrect")
    ...
    if len(my_task_list) == 0:
        ...
        update_task_menu(my_task_list,task_id)

        
def update_task_menu(t_list,id):
    # -- Reads all tasks  - whole list will be updated along with filtered list 
    read_task_list = read_tasks()
    while True:
        # -- User selects task ID to edit/update
        task_number = int(input("To edit task enter its id number: "))
        if task_number == id:
            # -- Task menu
            task_menu = ''' 
                Select your option:
    ...
    if edit_option == 'co':
        update_to_complete_task(t_list,id)
    ...
    # -- Option to edit username and/or due date in a task                
        ...
        while True:
            ...
            task_to_edit = [t for t in t_list if t['id'] == str(task_number) and t['completed'] == False]
            
            #-- List is non-empty, means not completed
            if task_to_edit:
                task_id = int((task_to_edit[0])['id'])
                
            # -- Task empty means it is already completed
            else:
...
def update_to_complete_task(task_list,task_number):
    read_task_list = read_tasks()
    # -- Filter user's task into a list using task number and completeness criteria
    task_to_complete = [t for t in task_list if t['id'] == str(task_number) and t['completed'] == False] 
...
def generate_tasks_report(t_list):
    ...
    completed_tasks = len([ct for ct in task_list if ct['completed'] == True])
    incomplete_tasks = total_tasks - completed_tasks
    
    # -- Working out all overdue tasks of users
    overdue_tasks = len([ovd_t for ovd_t in task_list if datetime.strptime(str(ovd_t['due_date'].date()),DATETIME_STRING_FORMAT) < today]) 
    ...
    task_report_list = []    
    task_report_list.append(new_task_report)
...
def generate_users_report(t_list):  
    ...
    for user in username_password:
        ...
        user_incomplete_task_overdue = 0  
        for tasks in t_list: 
            # -- Matches a user with a user registed in a task
            if user == tasks['username']:
                ...
                elif tasks['completed'] == False and datetime.strptime(str(tasks['due_date'].date()),DATETIME_STRING_FORMAT) < today:
                ...
                try:
                    user_task.update({"user":user_name,"user_total_tasks":user_total_tasks,
                                    ...
                                    "user_completed_tasks_percent":round(((user_total_completed_tasks/user_total_tasks)*100), 2),
                                    "user_incomplete_tasks_percent":round((((user_total_tasks- user_total_completed_tasks)/user_total_tasks)*100), 2),
                                    "user_incomplete_tasks_overdue_percent":round(((user_incomplete_task_overdue/user_total_tasks)*100), 2)})   
    ...
    headers = ["User","Tot tasks","Comp. tasks","Tot tasks %","Comp. tasks %","Incomp task %","Incomp & Overdue %"]
